...
#error natives-empty.cc is used only for empty snapshot build.
...
class NativesStore {
 public:
  ...
  int GetBuiltinsCount() { return 0; }
  int GetDebuggerCount() { return 0; }
...
//  Vector<const char> GetScriptName(int index) { return "native_names_[index]; }
  ...
  Vector<const char> GetScriptName(int index) { return Vector<const char>(); }
...
INSTANTIATE_TEMPLATES(D8)	//simon was here
